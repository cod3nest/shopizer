...
@Slf4j
@Service
class ContentServiceImpl extends SalesManagerEntityServiceImpl<Long, Content> implements ContentService {

    private final ContentRepository contentRepository;
    private final StaticContentFileManager contentFileManager;

    public ContentServiceImpl(ContentRepository contentRepository, StaticContentFileManager contentFileManager) {
    ...
    private void addImage(String merchantStoreCode, InputContentFile contentImage) throws ServiceException {
        ...
        try {
            LOGGER.info("Adding content image for merchant id {}", merchantStoreCode);
            ...
            LOGGER.error("Error while trying to convert input stream to buffered image", e);
    ...
    private void addFile(final String merchantStoreCode, InputContentFile contentImage) throws ServiceException {
        ...
        try {
            LOGGER.info("Adding content file for merchant id {}", merchantStoreCode);
    ...
    public void addContentFiles(String merchantStoreCode, List<InputContentFile> contentFilesList)
        ...
        LOGGER.info("Adding total {} images for given merchant", contentFilesList.size());

        String p = null;
        Optional<String> path = Optional.ofNullable(p);

        LOGGER.info("Adding content images for merchant....");
